...
import type { PORecord, POItemRecord, POStatus } from '@/lib/types/finance'
        ...
        <div className="rounded-[1.25rem] border border-stone-200 bg-white p-5 space-y-4">
          ...
          {isEditable ? (
            <form action={upsertPOItemsAction} className="space-y-4">
              <input type="hidden" name="po_id" value={po.id} />
              <LineItemsEditor
                initialItems={items.map(item => ({ id: item.id, product_id: item.product_id ?? '', description: item.description, quantity: String(item.quantity), unit_price: String(item.unit_price) }))}
                products={products.map(p => ({ id: p.id, name: p.name, price: p.price }))}
                isTaxable={po.is_taxable}
              />
              <SubmitButton className="rounded-lg bg-stone-950 px-4 py-2 text-sm font-medium text-white hover:bg-stone-800">
                Simpan Item
              </SubmitButton>
