...
export async function buildInvoicePDF(
  ...
  // ── Color palette ──────────────────────────────────────────────────────────
  const primary = hexToRgb(company?.color ?? '#1c1917')       // color_primary
  const secondary = hexToRgb(company?.color_secondary ?? '#44403c') // color_secondary
  const accent = hexToRgb(company?.color_accent ?? '#eab308')  // color_accent (yellow-ish default)
  ...
  // ── Reference numbers (right column in header) ─────────────────────────────
  const refX = 130
  ...
  addRef('No.', invoice.invoice_number)
  addRef('Tanggal', formatDate(invoice.invoice_date))
  if (invoice.due_date) addRef('Jatuh Tempo', formatDate(invoice.due_date))
  if (invoice.po_reference) addRef('PO. No', invoice.po_reference)
  if (invoice.quo_reference) addRef('Quo. No', invoice.quo_reference)
  if (invoice.spk_reference) addRef('SPK. No', invoice.spk_reference)
  ...
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.text('Ditujukan Kepada:', marginL, y)
  y += 5
  ...
  if (invoice.customer_address) {
    const addrLines = doc.splitTextToSize(invoice.customer_address, 100) as string[]
    doc.text(addrLines, marginL, y)
    y += addrLines.length * 4.5
  }
  if (invoice.customer_npwp) {
    doc.setFont('helvetica', 'bold')
    doc.text(`NPWP : ${invoice.customer_npwp}`, marginL, y)
    doc.setFont('helvetica', 'normal')
    y += 5
  }
  ...
  addTotalRow('Sub Total', formatRupiah(invoice.subtotal))

  if (invoice.tax_type === 'ppn') {
    addTotalRow('DPP (11/12)', formatRupiah(invoice.dpp))
    addTotalRow('PPN 12%', formatRupiah(invoice.tax_amount))
  } else if (invoice.tax_type === 'pph23') {
    addTotalRow('PPh 23 2%', `(${formatRupiah(invoice.tax_amount)})`, false, [180, 50, 50])
  }
  ...
  y += 1
  addTotalRow('Total', formatRupiah(invoice.total), true)
  ...
  doc.text(`Terbilang : ${terbilangRupiah(invoice.total)}`, marginL, y)
  y += 8
  ...
  if (invoice.notes) {
    ...
    const noteLines = doc.splitTextToSize(`*${invoice.notes}`, pageW - marginL * 2 - 80) as string[]
